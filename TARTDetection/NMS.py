...
def filter_bbox(bbox, width=0, height=0):
    xrng = [0, width]
    yrng = [0, height]
    # bbox[:, :4] = bbox[:, :4] / 1.5
    x1 = bbox[:, 0]
...
def filter_bbox_v2(bbox, width=0, height=0):
    xrng = [0, width]
    yrng = [0, height]
    x1 = bbox[:, 0]
    y1 = bbox[:, 1]
    x2 = bbox[:, 0] + bbox[:, 2]
    y2 = bbox[:, 1] + bbox[:, 3]
    keep = np.where((x1 < xrng[1]) & (x2 > xrng[0]) & (y1 < yrng[1]) & (y2 > yrng[0]))[0]
    bbox = bbox[keep, :]
    x1 = x1[keep]
    y1 = y1[keep]
    x2 = x2[keep]
    ...
    x2 = np.minimum(xrng[1], x2)
    y2 = np.minimum(yrng[1], y2)
    bbox[:, 0] = x1
    bbox[:, 1] = y1
    bbox[:, 2] = x2 - x1
    bbox[:, 3] = y2 - y1
